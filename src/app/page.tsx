...
const advantageItems = advantageGroups.flatMap((group) => group.items);
...
const modifierPalette = ["#111315", "#8da0b8", "#d7dde2", "#4b545c", "#74806f", "#b6a57d", "#7d2f2b"];

const modifierTrackOptions = [
  {
    id: "high",
...
function scrollToModifier(event: React.MouseEvent<HTMLAnchorElement>) {
    ...
    <header className="fixed left-0 right-0 top-0 z-50 px-4 pt-4">
      <div className="mx-auto flex h-16 max-w-[1440px] items-center justify-between rounded-[8px] border border-white/10 bg-carbon/58 px-4 shadow-glass backdrop-blur-2xl md:px-6">
        ...
        <nav className="hide-scrollbar hidden items-center gap-1 overflow-x-auto md:flex">
          ...
          <a href="#modifier" onClick={scrollToModifier} className="rounded-[8px] border border-ember/35 bg-ember/10 px-3 py-2 text-sm text-white transition hover:bg-ember/18">
            Модификатор
          </a>
        </nav>
    ...
    <section id="top" className="relative min-h-[100svh] overflow-hidden pb-10">
      ...
      <motion.div style={{ y, opacity }} className="section-shell relative z-20 flex min-h-[100svh] flex-col items-center justify-start pt-28 text-center sm:justify-center">
        ...
        <div className="relative z-30 mt-7 flex w-full max-w-xl flex-col gap-3 sm:mt-9 sm:w-auto sm:max-w-none sm:flex-row">
          ...
          <Button asChild size="lg" className="w-full sm:w-auto">
            <a href="#modifier" onClick={scrollToModifier}>Открыть модификатор <ArrowRight size={18} /></a>
          </Button>
          ...
          {advantageItems.map((item) => (
            <Reveal key={item.title} className="h-full">
              <Link href={item.href} className="group block h-full focus-visible:outline-none">
                <Card className="h-[216px] overflow-hidden border-white/8 bg-white/[0.035] transition-[transform,border-color,background-color,box-shadow] duration-300 ease-out hover:-translate-y-1 hover:border-ember/32 hover:bg-white/[0.055] hover:shadow-[0_22px_64px_rgba(0,0,0,0.32)] group-focus-visible:-translate-y-1 group-focus-visible:border-ember/32 group-focus-visible:bg-white/[0.055] md:h-[226px]">
                  <CardContent className="relative flex h-full flex-col">
                    <div className="mb-4 h-px w-10 bg-gradient-to-r from-ember to-transparent transition-all duration-300 group-hover:w-20 group-focus-visible:w-20" />
                    <h3 className="text-xl font-semibold leading-tight text-white md:text-[22px]">{item.title}</h3>
                    <div className="pointer-events-none absolute bottom-5 right-5 text-steel transition duration-300 group-hover:translate-x-1 group-hover:text-white group-focus-visible:translate-x-1 group-focus-visible:text-white">
                      <ChevronRight size={18} />
                    </div>
                    <div className="mt-auto translate-y-2 opacity-0 transition-[opacity,transform] duration-300 ease-out group-hover:translate-y-0 group-hover:opacity-100 group-focus-visible:translate-y-0 group-focus-visible:opacity-100">
                      <p className="pr-4 text-xs leading-5 text-white/66 md:text-[13px]">{item.text}</p>
                      <span className="mt-4 inline-flex items-center gap-2 text-xs font-semibold uppercase tracking-[0.16em] text-steel transition group-hover:text-white group-focus-visible:text-white">
                        Подробнее
                      </span>
                    </div>
                  </CardContent>
...
function PhotoPlaceholder() {
  return (
    <section className="relative bg-[#080a0b] py-24 md:py-36">
      <div className="section-shell">
        <div className="min-h-[52vh] rounded-[8px] border border-white/10 bg-white/[0.035] shadow-glass backdrop-blur-2xl" />
      </div>
    </section>
  );
}
...
function ModifierSection() {
  const [activeId, setActiveId] = useState(modifierVariants[0].id);
  const [activeTrackId, setActiveTrackId] = useState(modifierTrackOptions[0].id);
  const active = modifierVariants.find((variant) => variant.id === activeId) ?? modifierVariants[0];
  const activeTrack = modifierTrackOptions.find((track) => track.id === activeTrackId) ?? modifierTrackOptions[0];

  return (
    <section id="modifier" className="relative overflow-hidden bg-carbon py-24 md:py-36">
      ...
      <div className="section-shell relative z-10 grid gap-10 lg:grid-cols-[0.82fr_1.18fr] lg:items-center">
        <div>
      ...
      <div className="section-shell relative z-10 mt-10">
        <div className="relative overflow-hidden rounded-[8px] border border-white/10 bg-white/[0.035] p-5 shadow-glass backdrop-blur-2xl md:p-8">
          ...
          <div className="relative mt-8 grid gap-4 lg:grid-cols-2">
            {modifierTrackOptions.map((track) => {
    ...
    <section id="технологии" className="relative overflow-hidden bg-[#0a0c0d] py-24 md:py-36">
      ...
      <div className="section-shell grid gap-10 lg:grid-cols-[1fr_1.08fr] lg:items-center">
        ...
        <Reveal className="relative min-h-[520px] overflow-hidden rounded-[8px]">
          <Image src={`${BASE_PATH}/images/optimized/p41-hero-2.png`} alt="Снегоболотоход P-4" fill sizes="(min-width: 1024px) 50vw, 100vw" className="object-cover" loading="eager" />
        ...
        <Reveal className="mb-10 grid gap-6 lg:grid-cols-[0.9fr_1.1fr] lg:items-end">
          ...
          <div>
            ...
            <Button asChild className="mt-6">
              <a href="#modifier" onClick={scrollToModifier}>Выбрать модификацию <ChevronRight size={18} /></a>
            </Button>
          </div>
        </Reveal>
